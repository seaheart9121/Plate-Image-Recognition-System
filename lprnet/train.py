...
#     BASE_DIR = 'lpr_runs'
#     DATASET_ROOT = 'ccpd_plate_crop_dataset1'
...
def train_lpr(resume=False):
    ...
    # 权重备用路径：脚本 上一级 / lpr_run / lprnet_best.pth
    current_script_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(current_script_dir)
    fallback_weights_path = os.path.join(parent_dir, 'lpr_runs', 'lprnet_best.pth')
    ...
    # 优先级：exp → 上一级 lpr_run → 从零训练
    if prev_exp_num is not None:
        prev_best_weights = os.path.join(BASE_DIR, f'exp{prev_exp_num}', 'lprnet_best.pth')
        if os.path.exists(prev_best_weights):
            weights_to_load = prev_best_weights
            print(f"📌 加载上一轮权重: {weights_to_load}")
        else:
            print(f"⚠️ 上一轮权重不存在")
            if os.path.exists(fallback_weights_path):
                weights_to_load = fallback_weights_path
                print(f"📌 加载备用权重: {fallback_weights_path}")
    else:
        print(f"⚠️ 无历史训练记录")
        if os.path.exists(fallback_weights_path):
            weights_to_load = fallback_weights_path
    ...
    if weights_to_load and os.path.exists(weights_to_load):
        try:
            state_dict = torch.load(weights_to_load, map_location=DEVICE)
